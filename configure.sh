...
cmake ${BUILD_PATH}/.. -DCMAKE_INSTALL_PREFIX=${INSTALL_PATH} -DCMAKE_C_FLAGS=-fPIC -DCMAKE_CXX_FLAGS=-fPIC -DOPENVDB_BUILD_UNITTESTS=OFF -DBOOST_ROOT=${REZ_BOOST_ROOT} -DBLOSC_LOCATION=${REZ_BLOSC_ROOT} -DGLEW_LOCATION=${REZ_GLEW_ROOT} -DGLFW_LOCATION=${REZ_GLFW_ROOT}  -DTBB_LOCATION=${REZ_TBB_ROOT} -DILMBASE_LOCATION=${REZ_ILMBASE_ROOT} -DOPENEXR_LOCATION=${REZ_OPENEXR_ROOT}
